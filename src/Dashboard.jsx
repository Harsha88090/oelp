...
// Palette for 10 lines — distinct, ocean-inspired
...
export default function Dashboard() {
  ...
  const last10Dates = getLastNDates(10);

  // Build per-date deduplicated minute-level arrays
  ...
  // Collect all unique minute labels across 10 days for unified x-axis
  const allMinuteLabels = Array.from(
    new Set(
      perDateData.flatMap(d => d.points.map(p => p.displayTime))
    )
  ).sort((a, b) => {
    // sort by time value
  ...
  const handleZoomIn = () => {
    if (chartRef.current) { chartRef.current.zoom(1.3); setIsZoomed(true); }
  };
  const handleZoomOut = () => {
    if (chartRef.current) { chartRef.current.zoom(0.77); }
  };
  const handleResetZoom = () => {
    if (chartRef.current) { chartRef.current.resetZoom(); setIsZoomed(false); }
  };
  const toggleZoomMode = () => {
    setZoomMode(prev => (prev === "wheel" ? "drag" : "wheel"));
  };

  // ── Zoom handlers — cumulative chart ─────────────────────────────
  const handleCumZoomIn = () => {
    if (cumulativeChartRef.current) { cumulativeChartRef.current.zoom(1.3); setCumIsZoomed(true); }
  };
  const handleCumZoomOut = () => {
    if (cumulativeChartRef.current) { cumulativeChartRef.current.zoom(0.77); }
  };
  const handleCumResetZoom = () => {
    if (cumulativeChartRef.current) { cumulativeChartRef.current.resetZoom(); setCumIsZoomed(false); }
  };
  const toggleCumZoomMode = () => {
    setCumZoomMode(prev => (prev === "wheel" ? "drag" : "wheel"));
  };
  ...
  const chartOptions = makeChartOptions(
    `Sensor Trend — ${selectedDate}`,
    zoomMode,
    setIsZoomed
  );

  const cumulativeChartOptions = makeChartOptions(
    "Cumulative Trend — Last 10 Days",
    cumZoomMode,
    setCumIsZoomed
  );
    ...
    <div className="dashboard">
      ...
      <div className="card chart-card cumulative-card">
        ...
        <div className="zoom-controls">
          ...
          {cumIsZoomed && (
            <button className="reset-btn" onClick={handleCumResetZoom}>↺ Reset</button>
          )}
            ...
            {perDateData.map((d, i) => {
              ...
              return (
                <div className="ten-day-stat-row" key={d.date}>
                  <span
                    className="ten-day-dot"
                    style={{ background: TEN_DAY_COLORS[i % TEN_DAY_COLORS.length] }}
                  />
